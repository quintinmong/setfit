...
# 2. 复用已有的持久化六分类 MacBERT 底座 (避免重复计算)
...
for dlg in seed_dialogues:
    ...
    embeddings = encoder.encode(dlg["history"], show_progress_bar=False)  # 形状: [3, 768]
    X_temporal_list.append(embeddings)
    y_list.append(dlg["label"])

X_temporal = np.array(X_temporal_list)  # 形状: [样本数, 步长(3), 特征维度(768)]
...
class TemporalSignalGRU(nn.Module):
    def __init__(self, input_size=768, hidden_size=64, num_layers=1, num_classes=4):
...
# 7. 模型初始化、定义损失函数与优化器
model = TemporalSignalGRU(input_size=768, hidden_size=64, num_classes=4).to(device)
...
with torch.no_grad():
    for session in test_sessions:
        ...
        # 2. 转换为 Tensor 并升维匹配 Batch [1, seq_len, 768]
